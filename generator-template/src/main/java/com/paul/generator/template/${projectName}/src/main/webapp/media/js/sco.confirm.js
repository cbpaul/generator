...
;(function($, undefined) {
	...
	function Confirm(options) {
		...
		if (!$modal.length) {
			$modal = $('<div class="modal" id="' + this.options.target.substr(1) + '"><div class="modal-body inner"/><div class="modal-footer"> <a href="#" class="btn red" data-action="1">是</a> <a class="btn cancel" href="#" data-dismiss="modal">否</a></div></div>').appendTo(this.options.appendTo).hide();
			
		}
		if (typeof this.options.action == 'function') {
			var self = this;
			$modal.find('[data-action]').attr('href', '#').unbind('click');
